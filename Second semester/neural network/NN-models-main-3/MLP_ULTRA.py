...
class MLP:
    ...
    def forward(self, X):
        ...
        """
        if self.bias:
            self.z_in = self.W1[0, :] + np.dot(X, self.W1[1:, :])
        else:
            self.z_in = np.dot(X, self.W1)

        self.z = self.act_func_h(self.z_in)
        if self.bias:
            self.y_in = self.W2[0, :] + np.dot(self.z, self.W2[1:, :])
        else:
    ...
    def backward(self, X, y, output):
        ...
        if self.batch:
            ...
            self.h += self.h_q
            self.h0 += self.hidden_delta
            self.o_q = self.z.T.dot(self.output_delta)
            self.o += self.o_q
            self.o0 += self.output_delta
    ...
    def train(self, X, y, epochs, learning_rate):
        if self.batch:
            for epoch in range(epochs):
                ...
                if(self.bias):
                    self.delta_v0 = self.h0 * learning_rate * -1
                    self.delta_w0 = self.o0 * learning_rate * -1

                    self.delta_v = np.vstack((self.delta_v0, self.delta_v))
                    self.delta_w = np.vstack((self.delta_w0, self.delta_w))
            ...
            for epoch in range(epochs):
                ...
                for i in range(X.shape[0]):
                    ...
                    if(self.bias):
                        self.delta_v0 = (
                            self.hidden_delta * learning_rate * -1
                        )
                        self.delta_w0 = (
                            self.output_delta * learning_rate * -1
                        )
                        self.delta_v = np.vstack((self.delta_v0, self.delta_v))
                        self.delta_w = np.vstack((self.delta_w0, self.delta_w))
                    self.W1 += self.delta_v
